...
// require_once(get_stylesheet_directory().'/@custom-hook/loader.php');
...
/***********************************************************************************************
 * action
 * https://www.youtube.com/watch?v=qDWRGqBbUCs
 ***********************************************************************************************/

add_action('rest_api_init', 'register_add_callback_url_endpoint');

function register_add_callback_url_endpoint(){
    register_rest_route(
        'register/v1', // namespace 
        'receive-callback', //endpoint
        array( // 
        'methods' => 'POST',
        'callback' => 'register_add_callback',
    ));
}

function register_add_callback($request_data){
    $data  = array();
	...
	if(isset($email)  && isset($username) && isset($password) && isset($role)){
		$data['status'] = 'OK';
		...
		$data['status'] = 'ERROR';
		$data['message'] = 'Please provide name and password';
	}
